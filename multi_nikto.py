...
def run_nikto(webserver, output_dir):
    ...
    nikto_args = ('-host {0} -port {1} -output {2}'.format(ip, port, results_file))
    if service_name == 'https' or tunnel == 'ssl':
        nikto_args += ' -ssl'
    print('nikto ' + nikto_args + '\r\n\r\n')
    try:
        subprocess.call('nikto ' + nikto_args, shell=True)
    except KeyboardInterrupt:
        return


def run_nikto_on_webservers(csv_input, output_dir):
    """ Parse csv and then run nikto for each."""
    dir_exists(output_dir, True)
    webservers = parse_csv_for_webservers(csv_input)
    for webserver in webservers:
        run_nikto(webserver, output_dir)
...
if __name__ == '__main__':
    run_nikto_on_webservers(parse_args().input, parse_args().output)
