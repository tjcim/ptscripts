...
USAGE: python testssl_image.py <url> <output_dir> [-s <screenshot directory>]
"""
import os
import logging
import argparse
...
LOG = logging.getLogger("ptscripts.testssl_image")


def main(args):
    LOG.info("Running testssl on {}".format(args.url))
    command = "testssl.sh {url}".format(url=args.url)
...
def parse_args(args):
    parser = argparse.ArgumentParser(
        ...
        description='Capture testssl data and image.',
    )
